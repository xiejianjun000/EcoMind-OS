"""
青岛众瑞 (Zhongrui) 分析仪协议适配器

青岛众瑞智能仪器有限公司是专业的环境监测仪器制造商，
产品涵盖大气、水质、烟尘等监测设备。

支持的型号:
  - ZR-CEMS-200: 烟气在线监测系统
  - ZR-3000: 大气颗粒物监测仪
  - ZR-5000: 水质多参数监测仪

连接方式:
  - serial: RS-232/RS-485 串口 (默认 9600bps)
  - ethernet: 自定义 TCP 协议 (默认端口 8200)
...
class ZhongruiAdapter(ProtocolAdapter):
    """青岛众瑞 (Zhongrui) 分析仪协议适配器"""

    brand_name = "青岛众瑞"
    supported_models = ["ZR-CEMS-200", "ZR-3000", "ZR-5000"]
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info("青岛众瑞设备连接成功: %s", self.host or self.serial_port)
            return True
    ...
    async def _cmd(self, func: int) -> bytes:
        self._writer.write(bytes([0xFD, func, 0x00]))
        await self._writer.drain()
    ...
    async def disconnect(self):
        ...
        if self._writer:
            try:
                self._writer.write(bytes([0xFD, 0xFF, 0x00]))
                await self._writer.drain()
                self._writer.close()
                await self._writer.wait_closed()
            except Exception as e:
                logger.warning("青岛众瑞断开异常: %s", e)
            finally:
                self._writer = None
                self._reader = None
