"""
蓝盾光电 (Landun) 分析仪协议适配器

安徽蓝盾光电子股份有限公司是专业的光学分析仪器制造商。
其 CEMS 产品采用非分散红外(NDIR)和紫外差分吸收光谱(DOAS)技术。

支持的型号:
  - LD-CEMS-600: 烟气排放连续监测系统
  - LD-CEMS-800: 高精度烟气分析仪
  - LD-GAS-300: 多组分气体分析仪

连接方式:
  - serial: RS-232/RS-485 串口 (默认 19200bps)
  - ethernet: 自定义 TCP 协议 (默认端口 7000)
...
class LandunAdapter(ProtocolAdapter):
    """蓝盾光电 (Landun) 分析仪协议适配器"""

    brand_name = "蓝盾光电"
    supported_models = ["LD-CEMS-600", "LD-CEMS-800", "LD-GAS-300"]
    connection_types = [ConnectionType.SERIAL, ConnectionType.ETHERNET, ConnectionType.MODBUS_TCP]
    default_port = 7000

    def __init__(
        self, host=None, port=None, serial_port=None, baud_rate=19200,
    ...
    async def connect(self) -> bool:
        ...
        try:
            if self.connection_type in (ConnectionType.ETHERNET, ConnectionType.MODBUS_TCP):
                ...
                self._writer.write(bytes([0x02, 0x4C, 0x44, 0x03]))  # STX LD ETX
            ...
            logger.info("蓝盾光电设备连接成功: %s", self.host or self.serial_port)
            return True
    ...
    async def _cmd(self, cmd: int) -> bytes:
        """发送命令帧: STX + CMD + LEN + DATA + ETX"""
        self._writer.write(bytes([0x02, cmd, 0x00, 0x03]))
        await self._writer.drain()
        return await asyncio.wait_for(self._reader.read(256), timeout=self.timeout)

    async def read_parameters(self) -> dict:
        """读取蓝盾光电校准参数"""
        await self._ensure_connected()
        try:
            resp = await self._cmd(0x20)
            if len(resp) >= 20:
                return {
    ...
    async def disconnect(self):
        ...
        if self._writer:
            try:
                self._writer.write(bytes([0x02, 0xFF, 0x00, 0x03]))
                await self._writer.drain()
                self._writer.close()
                await self._writer.wait_closed()
            except Exception as e:
                logger.warning("蓝盾光电断开异常: %s", e)
            finally:
                self._writer = None
                self._reader = None
    ...
    async def check_hidden_menu(self) -> list[str]:
        ...
        try:
            ...
            if resp and len(resp) > 3 and resp[1] == 0x80:
                menus.append("工程模式 (cmd=0x80)")
            resp = await self._cmd(0x81)
            if resp and len(resp) > 3 and resp[1] == 0x81:
                menus.append("校准调试菜单 (cmd=0x81)")
        except Exception:
            pass
        return menus
