...
export const agentApi = {
  list: async (params?: { status?: string; provider?: string }): Promise<{ agents: import('@/services/types').AgentResponse[] }> => {
    try {
      ...
      const agents: import('@/services/types').AgentResponse[] = (data.agents || []).map((a: any) => ({
        agent_id: a.agent_id,
        name: a.display_name || a.agent_id,
        description: a.category || '',
        provider: 'deepseek',
        status: a.status === 'healthy' ? 'running' : a.status === 'idle' ? 'paused' : 'running',
        model: 'deepseek-chat',
        taiji_verify_enabled: true,
        tools: a.capabilities || [],
        created_at: new Date(Date.now() - (a.uptime_sec || 0) * 1000).toISOString(),
      ...
      console.warn('[agentApi.list] fetch failed, using empty list:', err);
      return { agents: [] };
    }
  },
